...
def main(args):
    ...
    for epoch in range(last_epoch + 1, args.max_epoch):
        ...
        for i, sample in enumerate(progress_bar):
            ...
            '''
            # We get the output of model by feeding the model with sample['src_tokens'], sample['src_lengths'], sample['tgt_inputs']
            # sample['src_tokens']: the sentence in source language.
            # sample['src_lengths']: the source language's sentences' lengths.
            # sample['tgt_inputs']: the input sentences in the target language.
            ...
            # The average training CrossEntropyLoss is calculated given prediction output and sample['tgt_tokens'] (i.e. label).
            loss = \
                criterion(output.view(-1, output.size(-1)), sample['tgt_tokens'].view(-1)) / len(sample['src_lengths'])
            # We use the loss to perform backpropagation for updating the model parameters.
            loss.backward()
            # The norm is computed over all gradients together with default clip threshold of gradients = 4.
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip_norm)
            # Updating the model parameters.
            optimizer.step()
            # Reset the gradients to zero at the end of each batch.
            optimizer.zero_grad()
