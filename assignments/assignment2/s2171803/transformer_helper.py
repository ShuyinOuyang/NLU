...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, state, encoder_padding_mask):
        ...
        # encoder_padding_mask.size =  [batch_size, seq_len],
        # where seq_len is the fix number in each batch that could unify all the sequence in to same length.
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # state.size(before) =  [tgt_time_steps, batch_size, embed_dim]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Get Q,K,V with size (batch size * num heads, target length, head_embed_size)
        Q = self.q_proj(query).contiguous().view(-1, batch_size, self.num_heads, self.head_embed_size).transpose(0, 2)
        K = self.k_proj(key).contiguous().view(-1, batch_size, self.num_heads, self.head_embed_size).transpose(0, 2)
        V = self.v_proj(value).contiguous().view(-1, batch_size, self.num_heads, self.head_embed_size).transpose(0, 2)
        ...
        # Q*K^T
        intermediate_result = torch.matmul(Q, K.transpose(1, 2)) / self.head_scaling

        #  masking
        if key_padding_mask is not None:  # avoid concerning about the padding elements
            # mask the attention weights
            key_padding_mask = key_padding_mask.unsqueeze(dim=1).repeat(self.num_heads, 1, 1)
            intermediate_result.masked_fill(key_padding_mask, float('-inf'))

        if attn_mask is not None:  # avoid concerning about the following words
            intermediate_result += attn_mask.unsqueeze(dim=0)

        # apply softmax
        attn_weights = F.softmax(intermediate_result, dim=-1)
        # apply dropout layer
        attn_weights = F.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        # proudction with V
        intermediate_result = torch.bmm(attn_weights, V)
        # change size fitted for output projection
        intermediate_result = intermediate_result.contiguous().view(self.num_heads, batch_size, -1, self.head_embed_size).transpose(0, 2)
        intermediate_result = intermediate_result.contiguous().view(-1, batch_size, self.num_heads * self.head_embed_size)
        # finally through the output projection, get attn with size [tgt_time_steps, batch_size, embed_dim]
        attn = self.out_proj(intermediate_result)
        # TODO: --------------------------------------------------------------------- CUT
