...
class LSTMEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if self.bidirectional:
            ...
            # final_hidden_states.size = [D * self.num layers, batch size, self.hidden size]
            # where D=1 if self.bidirectional== False, otherwise, D=2
            final_cell_states = combine_directions(final_cell_states)
            # final_cell_states = [D*self.num layers, batch size, self.hidden size],
            # where D=1 if self.bidirectional== False, otherwise, D=2
...
class AttentionLayer(nn.Module):
    ...
    def forward(self, tgt_input, encoder_out, src_mask):
        ...
        # attn_context = [batch_size, output_dims]
    ...
    def score(self, tgt_input, encoder_out):
        ...
        # projected_encoder_out = [batch_size, output_dims, src_time_steps]
        attn_scores = torch.bmm(tgt_input.unsqueeze(dim=1), projected_encoder_out)
        # attn_scores = [batch_size, 1, src_time_steps]
...
class LSTMDecoder(Seq2SeqDecoder):
    ...
    def __init__(self,
        ...
        if self.use_lexical_model:
            ...
            self.output_lexical_projection = nn.Linear(embed_dim, len(dictionary))
            # pass
            # TODO: --------------------------------------------------------------------- /CUT
    ...
    def forward(self, tgt_inputs, encoder_out, incremental_state=None):
        ...
        if cached_state is not None:
            ...
            # tgt_hidden_states.size = [len(self.layers), batch_size, self.hidden_size]
        ...
        for j in range(tgt_time_steps):
            ...
            if self.attention is None:
                ...
                # input_feed.size = [batch_size, self.hidden_size]
            else:
                ...
                if self.use_lexical_model:
                    ...
                    lexical_context_step = torch.tanh(self.lexical_context_projection(lexical_context)) + lexical_context
                    lexical_contexts.append(lexical_context_step)
                    # pass
                    # TODO: --------------------------------------------------------------------- /CUT
        ...
        if self.use_lexical_model:
            ...
            decoder_output += self.output_lexical_projection(lexical_output)
            # pass
            # TODO: --------------------------------------------------------------------- /CUT
