...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			...
			x_onehot = make_onehot(x[t], self.V.shape[1])
			# RNN architecture
			s[t] = sigmoid(self.V @ x_onehot + self.U @ s[t-1])
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			# onehot for x and d
			x_onehot = make_onehot(x[t], self.V.shape[1])
			d_onehot = make_onehot(d[t], self.V.shape[1])
	...
	def acc_deltas_np(self, x, d, y, s):
		...
		# last time step
		t = len(x) - 1
		d_onehot = make_onehot(d[0], self.V.shape[1])
		x_onehot = make_onehot(x[t], self.V.shape[1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			for step in range(1, steps + 1):
				...
				x_onehot_step = make_onehot(x[t-step], self.V.shape[1])
				self.deltaV += np.outer(delta_in_step, x_onehot_step)
	...
	def compute_loss(self, x, d):
		...
		for i in range(len(d)):
			word_onehot = np.zeros(self.V.shape[1])
			word_onehot[d[i]] = 1
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		np.save(save_path+'/rnn.U.npy', model.U)
		np.save(save_path+'/rnn.V.npy', model.V)
		np.save(save_path+'/rnn.W.npy', model.W)
	...
	if mode == "train-np":
		...
		save_path = data_folder + '/parameter_new' + '/q5_hdim%s_lookback%s_lr%s' % (hdim, lookback, lr)
		if not os.path.exists(save_path):
			os.makedirs(save_path)
		np.save(save_path + '/rnn.U.npy', model.U)
		np.save(save_path + '/rnn.V.npy', model.V)
