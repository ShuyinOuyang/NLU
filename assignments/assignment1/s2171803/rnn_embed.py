...
class RNN(object):
	...
	def __init__(self, vocab_size, hidden_dims, out_vocab_size, embedding_dim=50):
		'''
		...
		self.V = np.random.randn(self.hidden_dims, self.embed_dims)*np.sqrt(0.1)
		self.W = np.random.randn(self.out_vocab_size, self.hidden_dims)*np.sqrt(0.1)
		self.C =  np.random.randn(self.embed_dims, self.vocab_size)*np.sqrt(0.1)
		
		
		# matrices to accumulate weight updates
		self.deltaU = np.zeros((self.hidden_dims, self.hidden_dims))
		self.deltaV = np.zeros((self.hidden_dims, self.embed_dims))
		self.deltaW = np.zeros((self.out_vocab_size, self.hidden_dims))
		self.deltaC = np.zeros((self.embed_dims, self.vocab_size))
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			...
			# one-hot for each word x
			x_onehot = make_onehot(x[t], self.vocab_size)
			x_embed = self.C @ x_onehot
			# RNN architecture
			s[t] = sigmoid(self.V @ x_embed + self.U @ s[t-1])
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			g_prime = np.ones(len(y[t]))
			f_prime = grad(s[t])

			delta_out = (d_onehot - y[t]) * g_prime
			self.deltaW += np.outer(delta_out, s[t])

			delta_in = (self.W.T @ delta_out) * f_prime
			self.deltaV += np.outer(delta_in, x_embed)
			
			self.deltaC += self.V.T @ np.outer(delta_in, x_onehot)
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			self.V.T @ np.outer(delta_in, x_onehot)
			...
			for step in range(1, steps + 1):
				if step > t:
					continue
				f_prime_step = grad(s[t-step])
				delta_in_step = self.U.T @ delta_in_list[-1] * f_prime_step

				x_onehot_step = make_onehot(x[t-step], self.vocab_size)
				x_embed_step = self.C @ x_onehot_step 
				
				self.deltaV += np.outer(delta_in_step, x_embed_step)
				
				self.deltaC += self.V.T @ np.outer(delta_in_step, x_onehot_step)
				
				self.deltaU += np.outer(delta_in_step, s[t-step-1])
				delta_in_list.append(delta_in_step)
			##########################
			# --- your code here --- #
			##########################
	...
	def acc_deltas_bptt_np(self, x, d, y, s, steps):
		...
		for step in range(1, steps + 1):
			...
			self.V.T @ np.outer(delta_in_step, x_onehot_step)
	...
	def compute_loss(self, x, d):
		...
		# --- your code here --- #
		##########################
		# transfer each word in sequence d into onehot
		d_onehot = []
		for i in range(len(d)):
			word_onehot = np.zeros(self.vocab_size)
			word_onehot[d[i]] = 1
			d_onehot.append(word_onehot)
		d_onehot = np.array(d_onehot)
		y, _ = self.predict(x)
		loss = -np.sum(d_onehot * np.log(y))
		return loss
	...
	def compute_loss_np(self, x, d):
		...
		loss = -np.sum(d_onehot * np.log(y[-1]))
	...
	def compute_mean_loss(self, X, D):
		...
		mean_loss = 0.
		# total number of words in corpus D
		n_word = 0
		for i in range(len(D)):
			mean_loss += self.compute_loss(X[i], D[i])
			n_word += len(D[i])
		mean_loss = mean_loss/n_word
		##########################
		# --- your code here --- #
		##########################
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		bestU, bestV, bestW, bestC = self.U, self.V, self.W, self.C
		best_epoch = 0
		...
		for epoch in range(epochs):
			...
			if loss < best_loss:
				best_loss = loss
				bestU, bestV, bestW, bestC = self.U.copy(), self.V.copy(), self.W.copy(), self.C.copy()
				best_epoch = epoch
		...
		self.U, self.V, self.W, self.C = bestU, bestV, bestW, bestC
...
def PCA(X, k=2, standardise=True):
    # standardise data to have std deviation 1
    if standardise:
        X = X / (np.std(X, axis=0) + 1e-10)
    
    # got cov
    cov = np.cov(X.T)
    # get D principle components 
    eigvals, V = np.linalg.eig(cov)
    # get top K principle components, sorted by eigenvalue 
    # which is of squarred projections onto principle component
    V = V[:, np.argsort(eigvals)[:k]]
    reduced = np.dot((X - np.mean(X,axis=0)), V)	
	
    return reduced
   
   

def visualise_similar_embeddings(word_to_num: dict, C: np.ndarray):
	words = ['king', 'queen', 'man', 'woman', 
		  'red', 'blue', 'green', 'black',
		  'football', 'basketball', 'run', 'running']
	

	reduced = PCA(C[:, [word_to_num[w] for w in words]].T)	
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		best_loss = model.train(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback)
		...
		# save_path = data_folder + '/parameters' + 'test' % (hdim, lookback, lr)
		save_path = data_folder + '/parameter_new'+'/hdim%s_lookback%s_lr%s' % (hdim, lookback, lr)
		if not os.path.exists(save_path):
			os.makedirs(save_path)
		np.save(save_path+'/rnn_embed.U.npy', model.U)
		np.save(save_path+'/rnn_embed.V.npy', model.V)
		np.save(save_path+'/rnn_embed.W.npy', model.W)
		np.save(save_path+'/rnn_embed.C.npy', model.C)
	...
	if mode == "train-np":
		'''
		starter code for parameters estimation.
		change this to different values, or use it to get you started with your own testing class
		'''
		train_size = 50000
		dev_size = 1000
		...
		##########################
		model = RNN(vocab_size, hdim, vocab_size)
		best_loss = model.train_np(X_train, D_train, X_dev, D_dev, learning_rate=lr, back_steps=lookback)
		...
		if not os.path.exists(save_path):
			os.makedirs(save_path)
		np.save(save_path+'/rnn_embed_np.U.npy', model.U)
		np.save(save_path+'/rnn_embed_np.V.npy', model.V)
		np.save(save_path+'/rnn_embed_np.W.npy', model.W)
		np.save(save_path+'/rnn_embed_np.C.npy', model.C)
	...
	if mode == "predict-lm":
		...
		U,V,W,C = np.load(rnn_folder + "/rnn_embed.U.npy"), np.load(rnn_folder + "/rnn_embed.V.npy"), np.load(rnn_folder + "/rnn_embed.W.npy"), np.load(rnn_folder + "/rnn_embed.C.npy")
		vocab_size = len(C[0])
		hdim = len(U[0])
	...
	if mode == "predict-test":

		data_folder = sys.argv[2]
		rnn_folder = sys.argv[3]

		# get saved RNN matrices and setup RNN
		U, V, W, C = np.load(rnn_folder + "/rnn_embed.U.npy"), np.load(rnn_folder + "/rnn_embed.V.npy"), np.load(
			rnn_folder + "/rnn_embed.W.npy"), np.load(rnn_folder + "/rnn_embed.C.npy")
		vocab_size = len(C[0])
		print(vocab_size)
		hdim = len(U[0])
		...
		print("Dev mean loss %s" % (run_loss))
		...
		print("Test mean loss %s" % (run_loss))
		print("Unadjusted: %.03f" % np.exp(run_loss))
		print("Adjusted for missing vocab: %.03f" % np.exp(adjusted_loss))